...
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			if (tree == null){  // if the tree is null, create a new tree
				tree = new PointQuadtree<>(point, 0, 0, width, height);
			}
			else{
				tree.insert(point); // if the tree is not null, add that point to the tree
			}
			...
			// TODO: YOUR CODE HERE
			found = tree.findInCircle(mouseX, mouseY, dotRadius);
	...
	/**
	 * / Test different values for a test 2 case and use them to test if the points are being drawn
	 * test2
	 */
	private void test2() {
		found = null;
		tree = new PointQuadtree<Dot>(new Dot(400,300), 0,0,800,600); // start with A
		tree.insert(new Dot(150,45)); // B
		tree.insert(new Dot(250,55)); // C
		tree.insert(new Dot(200,55)); // D
		int bad = 0;
		bad += testFind(0,0,900,4,4,4);		// rect for all; circle for all; find all
		bad += testFind(400,300,10,4,3,1);	// rect for all; circle for A,B; find A
		bad += testFind(150,450,10,2,1,0);	// rect for all; circle for all; find B
		bad += testFind(250,550,10,2,1,0);	// rect for all; circle for all; find C
		bad += testFind(150,450,200,4,3,0);	// rect for all; circle for all; find B, C
		bad += testFind(140,440,10,2,1,0);	// rect for all; circle for A,B; find none
		bad += testFind(750,550,10,2,1,0);	// rect for A,B; circle for A; find none
	...
	public void handleKeyPress(char key) {
		...
		} else if (key == '2'){  // Try your test case with 2
			test2();
	}
		repaint();
	}
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		// TODO: YOUR CODE HERE
		// call fill oval on the points in the tree and get the points in x and y
		...
		//Check if all the trees have children, if so draw them, while increasing the level of the colors
		if(tree.hasChild(1)){
